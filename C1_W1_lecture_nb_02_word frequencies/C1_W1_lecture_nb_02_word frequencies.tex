\documentclass[11pt]{article}
    ...
    % Document parameters
    % Document title
    \title{C1\_W1\_lecture\_nb\_02\_word frequencies}
...
\PY{k+kn}{import} \PY{n+nn}{matplotlib}\PY{n+nn}{.}\PY{n+nn}{pyplot} \PY{k}{as} \PY{n+nn}{plt}              \PY{c+c1}{\PYZsh{} visualization library}
\PY{k+kn}{import} \PY{n+nn}{numpy} \PY{k}{as} \PY{n+nn}{np}                           \PY{c+c1}{\PYZsh{} library for scientific computing and matrix operations}
...
\tightlist
\item
  \texttt{process\_tweet()}: Cleans the text, tokenizes it into separate
  words, removes stopwords, and converts words to stems.
\item
  \texttt{build\_freqs()}: This counts how often a word in the `corpus'
  (the entire set of tweets) was associated with a positive label
  \texttt{1} or a negative label \texttt{0}. It then builds the
  \texttt{freqs} dictionary, where each key is a \texttt{(word,label)}
  tuple, and the value is the count of its frequency within the corpus
  of tweets.
\end{itemize}
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} download the stopwords for the process\PYZus{}tweet function}
\PY{n}{nltk}\PY{o}{.}\PY{n}{download}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{stopwords}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}

\PY{c+c1}{\PYZsh{} import our convenience functions}
...
As in the previous lab, we will be using the
\href{http://www.nltk.org/howto/twitter.html\#Using-a-Tweet-Corpus}{Twitter
dataset from NLTK}.

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{3}{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} select the lists of positive and negative tweets}
...
Number of tweets:  10000
    ...
    Next, we will build an array of labels that matches the sentiments of
our tweets. This data type works pretty much like a regular list but is
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} This prints a value}
\PY{k}{if} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{key1}\PY{l+s+s1}{\PYZsq{}} \PY{o+ow}{in} \PY{n}{dictionary}\PY{p}{:}
    \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{item found: }\PY{l+s+s2}{\PYZdq{}}\PY{p}{,} \PY{n}{dictionary}\PY{p}{[}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{key1}\PY{l+s+s1}{\PYZsq{}}\PY{p}{]}\PY{p}{)}
\PY{k}{else}\PY{p}{:}
    \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{key1 is not defined}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}

\PY{c+c1}{\PYZsh{} Same as what you get with get}
\PY{n+nb}{print}\PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{item found: }\PY{l+s+s2}{\PYZdq{}}\PY{p}{,} \PY{n}{dictionary}\PY{o}{.}\PY{n}{get}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{key1}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{o}{\PYZhy{}}\PY{l+m+mi}{1}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
...
\begin{Highlighting}[]
    \ControlFlowTok{for}\NormalTok{ y, tweet }\KeywordTok{in} \BuiltInTok{zip}\NormalTok{(yslist, tweets):}
        \ControlFlowTok{for}\NormalTok{ word }\KeywordTok{in}\NormalTok{ process\_tweet(tweet):}
\NormalTok{            pair }\OperatorTok{=}\NormalTok{ (word, y)}
\NormalTok{            freqs[pair] }\OperatorTok{=}\NormalTok{ freqs.get(pair, }\DecValTok{0}\NormalTok{) }\OperatorTok{+} \DecValTok{1}
\end{Highlighting}
\end{Shaded}

    As shown above, each key is a 2-element tuple containing a
\texttt{(word,\ y)} pair. The \texttt{word} is an element in a processed
tweet while \texttt{y} is an integer representing the corpus: \texttt{1}
for the positive tweets and \texttt{0} for the negative tweets. The
value associated with this key is the number of times that word appears
in the specified corpus. For example:
...
\end{verbatim}
    ...
    Now, it is time to use the dictionary returned by the
\texttt{build\_freqs()} function. First, let us feed our \texttt{tweets}
and \texttt{labels} lists then print a basic report:
...
\PY{c+c1}{\PYZsh{} check data type}
\PY{n+nb}{print}\PY{p}{(}\PY{l+s+sa}{f}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{type(freqs) = }\PY{l+s+si}{\PYZob{}}\PY{n+nb}{type}\PY{p}{(}\PY{n}{freqs}\PY{p}{)}\PY{l+s+si}{\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}

\PY{c+c1}{\PYZsh{} check length of the dictionary}
\PY{n+nb}{print}\PY{p}{(}\PY{l+s+sa}{f}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{len(freqs) = }\PY{l+s+si}{\PYZob{}}\PY{n+nb}{len}\PY{p}{(}\PY{n}{freqs}\PY{p}{)}\PY{l+s+si}{\PYZcb{}}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
...
\PY{c+c1}{\PYZsh{} list representing our table of word counts.}
...
\PY{c+c1}{\PYZsh{} loop through our selected words}
...
(e.g.~\texttt{:)} has 3691 counts in the positive while only 2 in the
